...
import { MonitoringEmailService, EventType, ImpactLevel, } from "../services/email.service.js";
const router = Router();
...
router.post("/webhook/email", async (req, res) => {
    try {
        const { to, subject, content, eventType = EventType.INFO, impactLevel, sourceApplication, metadata, attachments, customCredentials, } = req.body;
        ...
        if (impactLevel && !Object.values(ImpactLevel).includes(impactLevel)) {
            res.status(400).json({
                ...
                details: `Impact level must be one of: ${Object.values(ImpactLevel).join(", ")}`,
            });
        }
