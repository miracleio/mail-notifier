...
export class MonitoringEmailService {
    ...
    constructor(customConfig) {
        ...
        this.defaultConfig = {
            ...
            secure: customConfig?.secure || process.env.SMTP_SECURE === "true" || false,
        };
        this.initializeTransporter();
    }
    initializeTransporter(customCredentials) {
    ...
    createEmailTemplate(payload) {
        const { subject, content, eventType, impactLevel, sourceApplication, timestamp = new Date(), metadata, } = payload;
      ...
      <html>
        <head>
          <style>
            ...
            .header { background-color: ${styling.bgColor}; padding: 20px; border-bottom: 5px solid ${styling.color}; }
            .title { display: flex; align-items: center; margin: 0; }
            .event-type { font-size: 14px; font-weight: bold; background-color: ${styling.color}; color: white; padding: 4px 8px; border-radius: 4px; margin-right: 10px; }
            .content { padding: 20px; background-color: white; }
            .message { background-color: #f8f9fa; border-left: 4px solid ${styling.color}; padding: 15px; margin-bottom: 20px; }
        ...
        <body>
          <div class="container">
            <div class="header">
              <div class="title">
                <span style="font-size: 24px; margin-right: 10px;">${styling.icon}</span>
    ...
    async sendMonitoringEmail(payload) {
        try {
            ...
            const mailOptions = {
                from: `"System Monitor" <${payload.customCredentials?.user || this.defaultConfig.user}>`,
                to,
    ...
    async sendSuccessNotification(options) {
        return this.sendMonitoringEmail({
            ...options,
            eventType: EventType.SUCCESS,
        });
    }
    async sendWarning(options) {
        return this.sendMonitoringEmail({
            ...options,
            eventType: EventType.WARNING,
        });
    }
    async sendInfo(options) {
